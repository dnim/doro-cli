...
function synthesize(tones: Tone[], volumeMultiplier = 1.0): Uint8Array {
  ...
  for (const tone of tones) {
    ...
    for (let i = 0; i < samples && writeIndex < pcm.length; i += 1) {
      ...
      const shaped =
        raw * tone.volume * MASTER_GAIN * volumeMultiplier * envelope(t, samples / SAMPLE_RATE);
...
function makeClip(tones: Tone[], volumeMultiplier = 1.0): Buffer {
  return wavFromPcm8Mono(synthesize(tones, volumeMultiplier));
}

export function createWorkStartClip(volumeMultiplier = 1.0): Buffer {
...
export function createShortRestStartClip(volumeMultiplier = 1.0): Buffer {
...
export function createLongRestStartClip(volumeMultiplier = 1.0): Buffer {
...
export function createCompletionBeepClip(volumeMultiplier = 1.0): Buffer {
...
export function createResetBeepClip(volumeMultiplier = 1.0): Buffer {
  return makeClip(
